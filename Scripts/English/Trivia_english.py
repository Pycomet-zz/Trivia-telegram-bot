...
# This Bot reads from  a file named ENGLISH.CSV
...
def trivia_file():
        ...
        with open("english.csv", "r") as f:
                ...
                b, option1 = random.choice(quiz)
                c, option2 = random.choice(quiz)
                d, option3 = random.choice(quiz)                
...
def trivia_answer(call):
        ...
        if call.data == "start" and call.from_user.id not in Admins:
                bot.delete_message(chat_id, message_id)
                trivia_game(call.message)
                condition = True
        if call.data == "stop" and call.from_user.id not in Admins:
                stop_trivia(call.message)
                condition = False
        if call.data == "pause" and call.from_user.id not in Admins:
                condition = False
                pause_trivia(call.message)
        if call.data == "continue" and call.from_user.id not in Admins:
                condition = True
                trivia_game(call.message)
        if call.data == "stats":
